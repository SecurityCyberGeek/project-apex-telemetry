...
# --- PER-CAR FUEL MODEL (example: Shanghai GP 2026) ---
...
# to a higher-energy deployment profile; Oscar uses a more conservative strategy.
...
try:
    while True:
        ...
        lando_speed     = 260.0 + (math.sin(elapsed * 0.8) * 60.0)
        is_accelerating = math.cos(elapsed * 0.8) > 0.2
        torque_anomaly  = (lando_temp > 130.0) and is_accelerating

        lando_rh_mm = 32.0 + (1.5 * math.sin(elapsed * 4.0))
        lando_vz_ms = 0.15 * math.sin(elapsed * 15.0)

        if torque_anomaly:
            lando_rh_mm -= 5.5
            lando_vz_ms  = 0.6 + (0.1 * math.sin(elapsed * 25.0))
        ...
        oscar_speed = 250.0 + (math.cos(elapsed * 0.5) * 75.0)
        ...
        if int(elapsed) % 5 == 0 and elapsed % 1.0 < (1.0 / FREQUENCY):
            print(
                f"[t={elapsed:>6.1f}s] "
